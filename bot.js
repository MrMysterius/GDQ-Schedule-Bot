...
module.exports = class Bot {
  ...
  async checkEntrys(config, calendar) {
    ...
    await page.screenshot({ fullPage: true, path: "website-screenshot.png" });
    ...
    for (let i = 0; i < event.counter; i++) {
      ...
      startTime.timestamp = `${startTime.year}-${startTime.month + 1 < 10 ? "0" + (startTime.month + 1) : startTime.month + 1}-${startTime.day}T${
        startTime.hour < 10 ? "0" + startTime.hour : startTime.hour
      }:${startTime.minute < 10 ? "0" + startTime.minute : startTime.minute}:${startTime.second < 10 ? "0" + startTime.second : startTime.second}`;
      endTime.timestamp = `${endTime.year}-${endTime.month + 1 < 10 ? "0" + (endTime.month + 1) : endTime.month + 1}-${endTime.day}T${
        endTime.hour < 10 ? "0" + endTime.hour : endTime.hour
      }:${endTime.minute < 10 ? "0" + endTime.minute : endTime.minute}:${endTime.second < 10 ? "0" + endTime.second : endTime.second}`;
        ...
        res.data.items.map((event, i) => {
          try {
            calendarEntries.push({
              entryID: event.id,
              gameEventID: event.description.match(id_regex)[0],
              game: event.summary,
    ...
    function insertOrUpdate(calendarEntries, convertedEvents, googleCalendar, config, eventName) {
      ...
      convertedEvents.forEach((e) => {
        e.startTime.dateNumber = toDateNumber({
        ...
        e.endTime.dateNumber = toDateNumber({
          year: e.endTime.year,
          month: e.endTime.month,
          day: e.endTime.day,
          hour: e.endTime.hour,
      ...
      calendarEntries.forEach((e) => {
        ...
        date = date[0].split("-");
        time = time[0].split(":");
        e.startDateNumber = toDateNumber({
      ...
      convertedEvents.forEach((scrapedEvent) => {
        ...
        calendarEntries.forEach((calendarEvent) => {
          if (calendarEvent.gameEventID == scrapedEvent.id.toString()) {
            if (
              calendarEvent.startDateNumber != scrapedEvent.startTime.dateNumber ||
              calendarEvent.endDateNumber != scrapedEvent.endTime.dateNumber ||
